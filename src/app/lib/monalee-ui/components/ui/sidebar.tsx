...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <MLIconContainer icon={<PanelLeft />} size="base" />
...
const sidebarGroupLabelVariants = cva(
  'duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opa] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
        default: 'group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0',
      },
    },
    defaultVariants: {
      variant: 'default',
...
function SidebarMenuButton({
  ...
  // If tooltip is provided and sidebar is collapsed, wrap with tooltip
  // For simplicity, we'll just return the button without tooltip wrapper
  // A full implementation would use the Tooltip component
  return button;
}
